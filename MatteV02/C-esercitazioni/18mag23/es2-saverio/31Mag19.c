...
int main(int argc, char** argv) {
        ...
        {	/* processo figlio */
            ...
            if ((status & 0xFF) != 0)
            {
                ...
                while(read(pipeFN[0], &c.c3[j], 1)){ /*finché ci sono caratteri da leggere*/
                    if(c.c3[j]=='\n'){
                        c.c3[++j]=0; /*metto terminatore di linea*/
                ...
                printf("DEBUG:il figlio %d ha letto: %d %d %s\n", i + 1, c.c1, c.c2, c.c3);
    ...
    for(i=0; i<argc-1;i++){
        printf("DEBUG:Lettura %d-esima nel padre\n", i + 1);
        int ret_read = read(piped[i][0], &c, sizeof(c));
        printf("DEBUG:ritorno READ figlio %d: %d\n", i + 1, ret_read);
        printf("DEBUG:Linea letta nel figlio %i: %d %d %s\n", i+1, c.c1, c.c2, c.c3);
        for(j=0;;j++){
            if(c.c3[j]=='\n'){
                printf("DEBUG:trovato terminatore nel figlio %d-esimo\n", i + 1);
