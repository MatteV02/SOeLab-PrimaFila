...
{   
   ...
   char line[512];			/* array usato dal figlio per inviare la stringa al padre e array usato dal padre per ricevere stringa inviata dal figlio */
   int pidFiglio, status, ritorno;      /* per wait padre */
	...
	{   
		...
		printf("DEBUG-Figlio %d sta per iniziare a scrivere una serie di messaggi, sulla pipe dopo averli letti dal file passato come parametro\n", getpid());
        ...
        // while (read(fd, line, MSGSIZE)) /* il contenuto del file e' tale che in mess ci saranno 4 caratteri e il terminatore di linea */
		// {   
	...
	while (read(piped[0], &line_length, sizeof(line_length)))  /* questo ciclo avra' termine appena il figlio terminera' dato che la read senza piu' scrittore tornera' 0! */
	{ 	
        	/* dato che il figlio gli ha inviato delle stringhe, il padre le puo' scrivere direttamente con una printf */
        	read(piped[0], line, line_length);
