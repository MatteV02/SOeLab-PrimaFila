...
int main(int argc, char** argv) {
    ...
    if (argc >= 4) /* controllo sul numero di parametri: devono essere in numero minore o uguale a 3*/
    {
    ...
    if(argc>=2){
        if(argc==3){
            ...
            close(0);
            if((open(argv[2], O_RDONLY)) < 0){		/* ERRORE se non si riesce ad aprire in LETTURA il file */
