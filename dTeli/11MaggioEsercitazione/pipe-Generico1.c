...
{   
   ...
   int pidFiglio, status, ritorno;      /* per wait padre */
	...
	int lungezza=0;
    while (read(piped[0],&lungezza, sizeof(int)))  /* questo ciclo avra' termine appena il figlio terminera' dato che la read senza piu' scrittore tornera' 0! */
	{ 	
        	/* dato che il figlio gli ha inviato delle stringhe, il padre le puo' scrivere direttamente con una printf */
            printf("lungezza stringa:%i\n",lungezza);
            read(piped[0],&mess, lungezza);
        	printf("%d: %s\n", j, mess);
        	j++;
	}
